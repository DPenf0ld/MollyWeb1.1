...
function Nav() {
    ...
    return (
        <>
            <nav
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#0d0d0d]/95 backdrop-blur-md border-b border-white/5' : 'bg-transparent'
                    }`}
            >
                ...
                <div className="flex flex-wrap gap-0 mb-12 border-b border-white/10">
                    {releases.map((r, i) => (
                        <button
                            key={r.title}
                            onClick={() => setActive(i)}
                            className={`font-['Lato'] text-xs tracking-[0.2em] uppercase px-6 py-4 transition-all duration-200 border-b-2 -mb-px ${active === i
                        ...
                        >
                            <div style={{ aspectRatio: '1/1' }} className="overflow-hidden">
                                <img
                                    src={r.cover}
                                    alt={r.title}
                                    className={`w-full h-full object-cover transition-all duration-500 ${active === i ? '' : 'grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100'
                                        }`}
                                />
                            </div>
